...
const PRODUCT_TEMPERATURE = 0.78;
const NAMING_TEMPERATURE = 0.82;
const PASSPORT_TEMPERATURE = 0.32;
const REWRITE_TEMPERATURE = 0.24;
...
function countOccurrencesDeep(value, needle) {
  const strings = collectStringsDeep(value, []);
  const n = sanitizeText(needle);
  if (!n) return 0;
  return strings.reduce((acc, s) => acc + (s.includes(n) ? 1 : 0), 0);
}

function countPlaceholderHits(value) {
  return collectStringsDeep(value, []).reduce((acc, s) => {
    const matches = s.match(/\[ПРЕДПОЛОЖЕНИЕ\]/g);
    return acc + (matches ? matches.length : 0);
  }, 0);
}

function findEnglishLeaks(value) {
  const strings = collectStringsDeep(value, []);
  const hits = [];
  for (const s of strings) {
    for (const re of ENGLISH_LEAK_PATTERNS) {
      if (re.test(s)) {
        hits.push(s);
        break;
      }
  ...
  return uniqueList(hits).slice(0, 12);
}
...
function collectCandidateProductTerms(passport, chosenName, kernel) {
  const candidates = new Set();
  const baseName = sanitizeText(chosenName || passport?.short_passport?.name);
  if (baseName) candidates.add(baseName);

  const shortVariants = normalizeList(passport?.short_passport?.name_variants).map(v => sanitizeText(v));
  shortVariants.forEach(v => v && candidates.add(v));

  const seedTexts = [
    kernel?.invented_category,
    kernel?.product_thesis,
    passport?.short_passport?.category,
    passport?.short_passport?.uniqueness,
    passport?.short_passport?.key_idea
  ].map(sanitizeText).filter(Boolean).join(" ");

  const titleLike = seedTexts.match(/[А-ЯЁ][А-Яа-яЁё0-9\-]{2,}(?:\s[А-ЯЁ][А-Яа-яЁё0-9\-]{2,}){0,2}/g) || [];
  titleLike.forEach(v => {
    const t = sanitizeText(v);
    if (t && t.length <= 30) candidates.add(t);
  });

  return Array.from(candidates).filter(Boolean);
}

function detectConceptDrift(passport, chosenName, kernel) {
  const issues = [];
  const expectedName = sanitizeText(chosenName || passport?.short_passport?.name);
  const actualName = sanitizeText(passport?.short_passport?.name);

  if (expectedName && actualName && expectedName !== actualName) {
    issues.push(`Имя в short_passport (${actualName}) не совпадает с выбранным именем (${expectedName})`);
  }

  const allStrings = collectStringsDeep(passport, []).join("\n");
  const allowedTerms = collectCandidateProductTerms(passport, chosenName, kernel);

  const suspiciousQuoted = allStrings.match(/[«"]([^"»\n]{3,40})[»"]/g) || [];
  suspiciousQuoted.forEach(raw => {
    const t = raw.replace(/[«»"]/g, "").trim();
    if (!t) return;
    if (allowedTerms.some(a => a === t)) return;
    if (/\[ПРЕДПОЛОЖЕНИЕ\]/.test(t)) return;
    if (/год|руб|дней|этап|вариант|тест|claim|маркировка/i.test(t)) return;
    if (wordCount(t) <= 4 && /^[А-ЯЁ][А-Яа-яЁё0-9\-\s]+$/.test(t)) {
      issues.push(`Внутри паспорта всплывает другая сущность в кавычках: ${t}`);
    }
  });

  const referencesToName = expectedName ? countOccurrencesDeep(passport, expectedName) : 0;
  if (expectedName && referencesToName < 2) {
    issues.push(`Выбранное имя ${expectedName} почти не используется внутри полного паспорта`);
  }

  return uniqueList(issues);
}

function validateCriticalCompleteness(passport) {
  const issues = [];

  const requiredTextFields = [
    ["short_passport.pain", passport?.short_passport?.pain, 12],
    ["short_passport.uniqueness", passport?.short_passport?.uniqueness, 14],
    ["short_passport.key_idea", passport?.short_passport?.key_idea, 12],
    ["branding.story", passport?.branding?.story, 24],
    ["branding.promise", passport?.branding?.promise, 10],
    ["branding.identity_effect", passport?.branding?.identity_effect, 14],
    ["marketing.base_product", passport?.marketing?.base_product, 16],
    ["marketing.pricing.pricing_logic", passport?.marketing?.pricing?.pricing_logic, 14],
    ["production.shelf_life_method", passport?.production?.shelf_life_method, 8],
    ["regulatory.legal_notes", passport?.regulatory?.legal_notes, 8],
    ["commerce.unit_economics.notes", passport?.commerce?.unit_economics?.notes, 8]
  ];

  requiredTextFields.forEach(([path, value, minWords]) => {
    if (wordCount(value) < minWords) issues.push(`Слабое поле ${path}`);
  });

  if (!Array.isArray(passport?.marketing?.segments) || passport.marketing.segments.length < 3) {
    issues.push("Недостаточно сегментов в marketing.segments");
  }

  if (!Array.isArray(passport?.validation_plan) || passport.validation_plan.length < 6) {
    issues.push("validation_plan слишком короткий");
  }

  if (!Array.isArray(passport?.risks) || passport.risks.length < 5) {
    issues.push("risks слишком короткий");
  }

  ["visual", "audio", "smell", "tactile", "taste"].forEach(k => {
    const mod = passport?.sensory_block?.[k];
    if (!mod || wordCount(mod.desired_experience) < 12 || wordCount(mod.brand_role) < 8) {
      issues.push(`Слабая сенсорная модальность ${k}`);
    }
  });
...
function isTooCloseToExistingCategory(kernel) {
  ...
  if (commodityHits > 0 && wordCount(whyNew) < 20) {
    issues.push("Слабое объяснение, почему продукта ещё нет на рынке");
  }

  if (wordCount(breakthrough) < 16) {
    issues.push("Слишком слабая breakthrough-механика");
  }

  if (wordCount(physicalForm) < 12) {
    issues.push("Слишком слабое описание физической формы продукта");
  }

  const rituals = normalizeList(kernel?.new_rituals);
  if (rituals.length < 3) {
    issues.push("Недостаточно новых ритуалов");
  } else {
    const weakRituals = rituals.filter(r => WEAK_NOVELTY_PATTERNS.some(re => re.test(r)));
    if (weakRituals.length >= 2) {
      issues.push("Ритуалы слишком бытовые и не создают нового класса потребления");
    }
  }

  const usage = normalizeList(kernel?.usage_scenarios);
  if (usage.length >= 2) {
    const similarUsage = usage.filter(r => WEAK_NOVELTY_PATTERNS.some(re => re.test(r)));
    if (similarUsage.length >= 2) {
      issues.push("Usage scenarios выглядят как удобный usage-case, а не как новый продукт");
    }
  }

  const monetizable = normalizeList(kernel?.monetizable_value);
  if (monetizable.length < 3) {
    issues.push("Недостаточно монетизируемой ценности");
  }
...
function evaluateKernelQuality(kernel) {
  ...
  if (!hasConcreteMarkers(JSON.stringify(kernel))) {
    issues.push("В kernel мало конкретики и коммерческих маркеров");
  }
...
function evaluatePassportQuality(passport, chosenName, kernel) {
  const issues = [];

  const v = validatePassportV2(passport);
  if (!v.ok) issues.push(...v.missing.map(x => `Отсутствует или слабый блок: ${x}`));

  const placeholderCount = countPlaceholderHits(passport);
  if (placeholderCount > 12) {
    issues.push(`Слишком много [ПРЕДПОЛОЖЕНИЕ]: ${placeholderCount}`);
  }

  const englishLeaks = findEnglishLeaks(passport);
  if (englishLeaks.length) {
    issues.push(`Обнаружены англоязычные или машинные куски: ${englishLeaks.slice(0, 4).join(" | ")}`);
  }

  const placeholderGarbage = findPlaceholderGarbage(passport);
  if (placeholderGarbage.length) {
    issues.push(`Обнаружен placeholder-мусор: ${placeholderGarbage.slice(0, 5).join(" | ")}`);
  }

  issues.push(...detectConceptDrift(passport, chosenName, kernel));
  issues.push(...validateCriticalCompleteness(passport));
...
function buildProductKernelSystemPrompt() {
  return joinNonEmpty([
    ...
    "Нельзя выдавать просто удобную версию батончика, паштета, снека, перекуса, десерта или другой существующей категории.",
...
function buildPassportSystemPrompt() {
  return joinNonEmpty([
    ...
    "7) Не используй англоязычные сенсорные слова вроде olfactive, tactile, gustatory и подобные вставки.",
...
function buildRegenerationSystemPrompt() {
  return joinNonEmpty([
    "Ты — старший редактор и product strategist.",
    "Ты пересобираешь СЛАБЫЙ product passport в сильный.",
    "Обязательно:",
    "1) Держи ОДНО выбранное имя и ОДНУ продуктовую сущность.",
    "2) Убери противоречия между названием и содержанием.",
    "3) Минимизируй [ПРЕДПОЛОЖЕНИЕ] в ключевых блоках.",
    "4) Убери англоязычные машинные вставки.",
    "5) Убери placeholder-мусор.",
    "6) Верни только полный JSON в той же схеме."
  ]);
}
...
function hardFallbackKernel(input) {
  return {
    ...
    product_thesis: `[ПРЕДПОЛОЖЕНИЕ] Продукт создаёт новую микрокатегорию за счёт новой физической формы, новой механики потребления и новой эмоционально-функциональной ценности.`,
    why_new: `[ПРЕДПОЛОЖЕНИЕ] В текущей категории нет массовых решений, которые одновременно меняют физическую форму продукта, сценарий потребления и объяснимую дополнительную ценность.`,
    why_not_existing_yet: `[ПРЕДПОЛОЖЕНИЕ] Продукт не стал массовым раньше из-за инерции категории, производственных ограничений и отсутствия явного бренд-нарратива для нового сценария.`,
    ...
    what_existing_solutions_fail_at: [
      ...
      "[ПРЕДПОЛОЖЕНИЕ] Не дают дополнительной монетизируемой ценности поверх базовой функции."
    ],
    breakthrough_mechanism: `[ПРЕДПОЛОЖЕНИЕ] Продукт соединяет новый объект, новый паттерн употребления и новый способ объяснить ценность в одной системе.`,
    ...
    portion_logic: `[ПРЕДПОЛОЖЕНИЕ] Порция подстроена под отдельный контекст и отдельную ценность, а не только под удобство.`,
...
function fallbackPassportV2(input, kernel, chosenName) {
  return {
    ...
    cognitive_block: {
      ...
      rituals: {
        behavior_change: "[ПРЕДПОЛОЖЕНИЕ] Новый продукт меняет поведение за счёт более короткого, понятного и предметно нового сценария.",
        old_vs_new_rituals: [
      ...
      narratives: {
        headlines: [
          "[ПРЕДПОЛОЖЕНИЕ] Когда вы выбираете этот продукт, вы получаете новый способ решать задачу без старых компромиссов.",
          "[ПРЕДПОЛОЖЕНИЕ] Когда продукт встраивается в день, бытовая боль превращается в новый устойчивый ритуал.",
          "[ПРЕДПОЛОЖЕНИЕ] Когда объект сам объясняет ценность, повторная покупка становится естественной."
        ],
        landing_copy: [
          "[ПРЕДПОЛОЖЕНИЕ] Продукт не просто закрывает базовую функцию категории, а создаёт новый ритуал и новую дополнительную ценность.",
          "[ПРЕДПОЛОЖЕНИЕ] Здесь соединяются предметная польза, сценарная ясность и новый поведенческий смысл."
        ],
        brand_story: "[ПРЕДПОЛОЖЕНИЕ] Бренд вводит новый продукт в жизнь человека как более умный и более современный способ решать ежедневную задачу."
      },
      desired_model: {
        thoughts: [
          "[ПРЕДПОЛОЖЕНИЕ] Я выбираю более продуманное решение.",
          "[ПРЕДПОЛОЖЕНИЕ] Это не компромисс, а новый класс продукта.",
      ...
      education: {
        ...
        kpi: ["[ПРЕДПОЛОЖЕНИЕ] QR scans", "[ПРЕДПОЛОЖЕНИЕ] CTR", "[ПРЕДПОЛОЖЕНИЕ] Sample-to-purchase conversion"]
      },
    ...
    sensory_block: {
      visual: {
        ...
        protocol_test: "[ПРЕДПОЛОЖЕНИЕ] Mockup test, N=30",
        analysis_method: "[ПРЕДПОЛОЖЕНИЕ] Средний балл и вербатимы",
      ...
      audio: {
        ...
        memory_marker: "[ПРЕДПОЛОЖЕНИЕ] Звук открытия или взаимодействия.",
        brand_role: "[ПРЕДПОЛОЖЕНИЕ] Делает ритуал узнаваемым.",
        measurable_goal: "[ПРЕДПОЛОЖЕНИЕ] satisfaction ≥ 7/9",
        protocol_test: "[ПРЕДПОЛОЖЕНИЕ] In-use test",
        analysis_method: "[ПРЕДПОЛОЖЕНИЕ] Оценочные шкалы",
        acceptance_criteria: "[ПРЕДПОЛОЖЕНИЕ] ≥7/9",
      ...
      smell: {
        ...
        protocol_test: "[ПРЕДПОЛОЖЕНИЕ] Blind panel, N=30",
        analysis_method: "[ПРЕДПОЛОЖЕНИЕ] Средний балл",
      ...
      tactile: {
        ...
        measurable_goal: "[ПРЕДПОЛОЖЕНИЕ] residue < threshold",
        protocol_test: "[ПРЕДПОЛОЖЕНИЕ] Contact test",
        analysis_method: "[ПРЕДПОЛОЖЕНИЕ] Измерение остатка",
        acceptance_criteria: "[ПРЕДПОЛОЖЕНИЕ] PASS if below threshold",
        recommendations: ["[ПРЕДПОЛОЖЕНИЕ] Soft-touch", "[ПРЕДПОЛОЖЕНИЕ] Контроль липкости и скольжения"]
      },
      taste: {
        ...
        analysis_method: "[ПРЕДПОЛОЖЕНИЕ] Средний балл и профиль",
        acceptance_criteria: "[ПРЕДПОЛОЖЕНИЕ] ≥7/9",
    ...
    branding: {
      ...
      customer_journey: [
        { stage: "awareness", customer_feeling: "[ПРЕДПОЛОЖЕНИЕ] Интерес", next_trigger: "[ПРЕДПОЛОЖЕНИЕ] Необычный формат или обещание" },
    ...
    marketing: {
      segments: [
        {
          segment: "[ПРЕДПОЛОЖЕНИЕ] Главный ранний сегмент",
          need: "[ПРЕДПОЛОЖЕНИЕ] Понятная польза плюс удобство",
          purchase_trigger: "[ПРЕДПОЛОЖЕНИЕ] Решение конкретной боли",
          why_they_buy: "[ПРЕДПОЛОЖЕНИЕ] Получают новый сценарий",
          acquisition_channel: "[ПРЕДПОЛОЖЕНИЕ] Ритейл или контент"
        },
        {
          segment: "[ПРЕДПОЛОЖЕНИЕ] Второй релевантный сегмент",
        ...
        {
          segment: "[ПРЕДПОЛОЖЕНИЕ] Третий релевантный сегмент",
      ...
      line_extension: ["[ПРЕДПОЛОЖЕНИЕ] Дальнейшее расширение линейки", "[ПРЕДПОЛОЖЕНИЕ] Второй продуктовый вариант"],
      pricing: {
        ...
        models: ["[ПРЕДПОЛОЖЕНИЕ] Розница", "[ПРЕДПОЛОЖЕНИЕ] Набор или подписка"]
      },
      channels: {
    ...
    production: {
      technology_process_steps: [
        ...
        { step: "[ПРЕДПОЛОЖЕНИЕ] Формование или сборка", detail: "[ПРЕДПОЛОЖЕНИЕ]", ccp: "[ПРЕДПОЛОЖЕНИЕ]" },
        { step: "[ПРЕДПОЛОЖЕНИЕ] Термообработка или стабилизация", detail: "[ПРЕДПОЛОЖЕНИЕ]", ccp: "[ПРЕДПОЛОЖЕНИЕ]" },
      ...
      microbiology_limits: {},
      production_constraints: ["[ПРЕДПОЛОЖЕНИЕ] Одно ключевое ограничение", "[ПРЕДПОЛОЖЕНИЕ] Второе ключевое ограничение"],
      source_evidence_assumptions: "[ПРЕДПОЛОЖЕНИЕ]"
    },
    regulatory: {
      allowed_claims: ["[ПРЕДПОЛОЖЕНИЕ] Разрешённый claim", "[ПРЕДПОЛОЖЕНИЕ] Второй разрешённый claim"],
      forbidden_claims: ["[ПРЕДПОЛОЖЕНИЕ] Нельзя обещать медицинский эффект", "[ПРЕДПОЛОЖЕНИЕ] Нельзя обещать недоказанный результат"],
    ...
    validation_plan: [
      { name: "[ПРЕДПОЛОЖЕНИЕ] Сенсорный тест", hypothesis: "[ПРЕДПОЛОЖЕНИЕ]", method: "[ПРЕДПОЛОЖЕНИЕ]", kpi: "[ПРЕДПОЛОЖЕНИЕ]", threshold: "[ПРЕДПОЛОЖЕНИЕ]", duration_days: 14, budget_rub: "[ПРЕДПОЛОЖЕНИЕ]" },
      { name: "[ПРЕДПОЛОЖЕНИЕ] In-use test", hypothesis: "[ПРЕДПОЛОЖЕНИЕ]", method: "[ПРЕДПОЛОЖЕНИЕ]", kpi: "[ПРЕДПОЛОЖЕНИЕ]", threshold: "[ПРЕДПОЛОЖЕНИЕ]", duration_days: 7, budget_rub: "[ПРЕДПОЛОЖЕНИЕ]" },
      { name: "[ПРЕДПОЛОЖЕНИЕ] Пилот", hypothesis: "[ПРЕДПОЛОЖЕНИЕ]", method: "[ПРЕДПОЛОЖЕНИЕ]", kpi: "[ПРЕДПОЛОЖЕНИЕ]", threshold: "[ПРЕДПОЛОЖЕНИЕ]", duration_days: 30, budget_rub: "[ПРЕДПОЛОЖЕНИЕ]" },
      { name: "[ПРЕДПОЛОЖЕНИЕ] Тест лендинга", hypothesis: "[ПРЕДПОЛОЖЕНИЕ]", method: "[ПРЕДПОЛОЖЕНИЕ]", kpi: "[ПРЕДПОЛОЖЕНИЕ]", threshold: "[ПРЕДПОЛОЖЕНИЕ]", duration_days: 14, budget_rub: "[ПРЕДПОЛОЖЕНИЕ]" }
    ],
    risks: [
    ...
    appendices: {
      ...
      qa_checklist_template: ["[ПРЕДПОЛОЖЕНИЕ] Проверка 1", "[ПРЕДПОЛОЖЕНИЕ] Проверка 2", "[ПРЕДПОЛОЖЕНИЕ] Проверка 3"]
    },
    sign_off_checklist: [
...
function collectWeakPathsV2(passport) {
  ...
  const check = (path, text, minWords) => {
    if (isWeakText(text, minWords)) weak.push({ path, text: sanitizeText(text) });
  };
...
async function regenerateBadPassportV2(passport, input, kernel, chosenName, qualityIssues) {
  const follow = await callTextModel([
    { role: "system", content: buildRegenerationSystemPrompt() },
    {
      role: "user",
      content: joinNonEmpty([
        `Выбранное имя продукта: ${chosenName}`,
        "Текущий паспорт считается слабым по этим причинам:",
        qualityIssues.map(x => `- ${x}`).join("\n"),
        "",
        "Текущий kernel:",
        JSON.stringify(kernel, null, 2),
        "",
        "Текущий слабый passport:",
        JSON.stringify(passport, null, 2),
        "",
        "Пересобери ВЕСЬ паспорт заново в той же полной схеме. Держи одно имя и одну сущность."
      ])
    }
  ], 0.30);

  if (!follow || typeof follow !== "object") return passport;

  let next = normalizePassportV2(follow, input, kernel, chosenName);
  next = await rewriteWeakSectionsV2(next);
  return next;
}

async function regenerateWeakKernel(kernel, input, qualityIssues) {
  const follow = await callTextModel([
  ...
  if (!follow || typeof follow !== "object") return kernel;
  return follow;
}
...
async function createProductKernel(input) {
  ...
  if (!quality.ok) {
    kernel = await regenerateWeakKernel(kernel, input, quality.issues);
  }
  ...
  const quality2 = evaluateKernelQuality(kernel);
  if (!quality2.ok) {
    console.warn("[kernel] still weak after repair:", quality2.issues);
...
async function createPassportV2(input, kernel, chosenName) {
  ...
  const quality = evaluatePassportQuality(passport, chosenName, kernel);
  if (!quality.ok) {
    console.warn("[passport_v2] quality fail:", quality.issues);
    passport = await regenerateBadPassportV2(passport, input, kernel, chosenName, quality.issues);
  }

  const quality2 = evaluatePassportQuality(passport, chosenName, kernel);
  if (!quality2.ok) {
    console.warn("[passport_v2] still imperfect after regeneration:", quality2.issues);
  }
