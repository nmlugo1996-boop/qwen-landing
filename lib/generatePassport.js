...
const CONCEPT_TEMPERATURE = 0.58;
...
function buildConceptSystemPrompt() {
  return joinNonEmpty([
    "Ты создаёшь новый продукт для заказчика, который любит яркие и конкретные паспорта.",
    ...
    "Название должно быть русским, коротким, запоминающимся, без латиницы и без пустых слов.",
...
function buildPassportSystemPrompt(enabledSchemas, include) {
  ...
  return joinNonEmpty([
    ...
    "Каждый ответ должен быть привязан к одному и тому же продукту. Нельзя уходить в общие слова. Нельзя писать методические комментарии и мета-текст.",
