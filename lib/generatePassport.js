...
// ⚡ ТЕМПЕРАТУРЫ
const CONCEPT_TEMPERATURE = 0.40;
...
const BAD_PHRASES = [
  // фразы, которые делают текст размытым и мешают конкретике
  "занятые потребители",
  "понятная выгода",
  "удобный формат",
  "новый и удобный",
...
// ⚡ ПРОМПТЫ — (оставляем вашу структуру, с встроенной METHODOLOGY_REFERENCE)
function buildConceptSystemPrompt() {
  return joinNonEmpty([
    ...
    "3. НАЗВАНИЕ НА РУССКОМ — короткое, звучное, без латиницы.",
    "4. ОПИШИ РИТУАЛ — где, когда, как именно человек ест этот продукт.",
    ...
    "ХОРОШО: 'Двухслойная мягкая колбасная пластина-рулет ... (корректный пример)'",
    "ПРОВЕРКА ПЕРЕД ОТВЕТОМ: Если нет конкретных цифр или предметного образа — ответ плохой.",
...
function buildPassportSystemPrompt(enabledSchemas, include) {
  ...
  return joinNonEmpty([
    ...
    "ТРЕБОВАНИЯ К КАЧЕСТВУ: конкретика, цифры, предметность, сценарии употребления.",
    "СТРУКТУРА ОТВЕТА: Когнитивный блок; Сенсорный блок; Брендинг; Маркетинг; Доп. секции.",
    "ЗАПРЕЩЕНО: пустые фразы и абстракции.",
...
// ⚡ FALLBACK и вспомогательная логика (без изменений по смыслу)
