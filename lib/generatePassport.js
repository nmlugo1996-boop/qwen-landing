...
const CONCEPT_TEMPERATURE = 0.6;
const PASSPORT_TEMPERATURE = 0.35;
const POLISH_TEMPERATURE = 0.22;
...
const BLOCK_SCHEMAS = {
  cognitive: [
    ...
    },
    {
      no: "1.3",
      question:
        "Изменение технологии потребления: какие новые привычки и ритуалы потребления внедряем?"
    },
    {
      no: "1.4",
      question: "Нарративы: как объясняем, что инновация нужна, полезна, выгодна?"
    },
    {
      no: "1.5",
      question: "Какие способы, каналы и приёмы обучения потребителей используем?"
    }
  ],
  sensory: [
  ...
  branding: [
    ...
    },
    {
      no: "3.3",
      question:
        "Сильное ядро бренда: название, логотип, слоган, уникальные дополнительные атрибуты"
    },
    {
      no: "3.4",
...
const BAD_PHRASES = [
  "новый продуктовый объект",
  "сценарный формат",
  "переупаковка категории",
  "удобный порционный формат",
  "компактные порции с понятной геометрией",
  "продукт должен быть",
  "нужно продумать",
  "нужно сделать",
  "можно сделать",
...
const METHODOLOGY_REFERENCE = truncateText(
  safeReadAny("methodology_meatcode.md", "meatcode.md"),
  16000
);
const PASSPORT_PROMPT_REFERENCE = truncateText(
  safeReadAny("passport_prompt.txt"),
  12000
);
const PASSPORT_SCHEMA_REFERENCE = truncateText(
  safeReadAny("passport_schema.json"),
  6000
);
const SHOKOVSYANKA_REFERENCE = truncateText(
  safeReadAny("shokovsyanka_march_2026.md"),
  12000
);
const GOOD_PASSPORT_REFERENCE = truncateText(
  safeReadAny("good_passport.json"),
  9000
);
...
function cleanupAnswer(text) {
  return sanitizeText(text)
    ...
    .replace(
      /\b(человек использует его через)\s+(человек)\b/gi,
      "использование: $2"
    )
    .trim();
}
...
function compressAudience(input) {
  ...
  if (Array.isArray(input.audienceList)) {
    rawAudience.push(...input.audienceList);
  }
  ...
  if (/(семь|родител|дет|дом|школ)/.test(joined)) {
    segments.push(
      "семьи с детьми, которым нужен быстрый и понятный продукт без лишней подготовки"
    );
  }
  if (/(офис|работ|дорог|перекус|машин|метро|с собой)/.test(joined)) {
    segments.push(
      "занятые взрослые, которым нужен удобный продукт для дороги, офиса или быстрого перекуса"
    );
  }
  if (/(спорт|фитнес|зал|белок)/.test(joined)) {
    segments.push(
      "активные взрослые, которым нужен функциональный продукт с понятной пользой"
    );
  }
  if (/(60\+|пожил|мягк|жеван|глот)/.test(joined)) {
    segments.push(
      "пожилые потребители, которым важны мягкость, удобство и простота использования"
    );
  }

  if (!segments.length) {
    const comment = `${sanitizeText(input.comment)} ${sanitizeText(
      input.pain
    )}`.toLowerCase();

    if (/(семь|дет|дом)/.test(comment)) {
      segments.push(
        "семьи с детьми, которым нужен быстрый и аккуратный продукт для дома"
      );
    } else {
      segments.push(
        "занятые потребители, которым нужен новый и удобный продукт с понятной выгодой"
      );
    }
  }
...
function buildConceptSystemPrompt() {
  return joinNonEmpty([
    "Ты создаёшь новый продукт для заказчика, который любит паспорта в стиле референсов 'Шоковсянка' и good_passport.",
    "Сначала придумай ОДИН новый продукт как физический объект. Не описывай категорию. Не перефразируй вопрос.",
    "Нужен не SKU-перекрас, а предмет с новой механикой использования, новым образом, новым ритуалом и понятным поводом попробовать.",
    "Название должно быть русским, коротким, запоминающимся, без латиницы и без пустых слов вроде 'Верный Вкус'.",
    "Тон должен быть как в одобренных заказчиком паспортах: яркий, конкретный, коммерчески понятный, без академической воды.",
...
function buildPassportSystemPrompt(enabledSchemas, include) {
  ...
  return joinNonEmpty([
    "Ты пишешь уже не концепт, а полный паспорт продукта в стиле одобренных заказчиком примеров.",
    "Требования к тону: ярко, коммерчески, конкретно, с новыми ритуалами, нарративами, самоисторией потребителя и понятным go-to-market.",
    "Требования к качеству: каждый ответ должен быть привязан к одному и тому же продукту. Нельзя уходить в общие слова. Нельзя писать методические комментарии и мета-текст.",
    "Используй стиль референсов: новый продукт + сильная боль + повод попробовать + образ продукта + доп. ценность + ритуал + бренд + развитие линейки.",
    ...
    extraSections.length
      ? `Дополнительные секции: ${extraSections.join(", ")}`
      : "",
    STYLE_REFERENCE
  ]);
}
...
function buildPolishSystemPrompt() {
  return joinNonEmpty([
    "Ты — редактор продукта. Твоя задача — не менять продукт, а отредактировать паспорт в более чистый и сильный вид.",
    "Убери повторы, мусорные формулировки, двойные слова, пустые названия, служебную бюрократию.",
    "Сделай текст ближе к одобренным заказчиком референсам: плотнее, смелее, вкуснее, но без потери конкретики.",
    "Не ломай структуру JSON. Верни только валидный JSON той же структуры."
  ]);
}

function buildPolishUserMessage(draft) {
...
function fallbackConcept(input) {
  ...
  if (/колбас/.test(lower)) {
    return {
      ...
      uniqueness:
        "Семейная кассета с двухслойными мясными медальонами: каждая порция уже собрана как готовый кусок с ярким вкусовым центром, который не надо резать, делить и дорабатывать.",
      one_liner:
        "Двухслойные мясные медальоны в кассете, где каждый кусок уже готов как отдельный семейный выбор.",
      physical_form: "овальные мягкие медальоны в отдельных отрывных ячейках",
      appearance:
        "ровные мясные медальоны с контрастным цветным центром на разрезе",
      composition:
        "внешний мясной слой и внутренний вкусовой центр: сливочный, травяной, томатный или горчичный",
      usage:
        "открыть кассету, оторвать ячейку, снять плёнку и сразу положить порцию на хлеб, в ланчбокс или на тарелку",
      novelty_mechanism:
        "новизна не в упаковке, а в самой конструкции порции: продукт уже разделён, собран и вкусово закончён внутри",
      why_people_will_try_it:
        "Он выглядит как новая вещь на полке и обещает разный вкус в одной семейной упаковке без ножа и доски.",
      new_market:
        "рынок семейных готовых мясных порций, где покупают не палку колбасы, а набор отдельных готовых решений",
      replacement_logic:
        "заменяет обычную варёную колбасу для дома, которую нужно резать, хранить и доедать",
      ritual:
        "каждый член семьи выбирает свой медальон и свой вкус из общей кассеты",
      ...
      launch_hook:
        "показать семью, где у каждого из одной кассеты свой вкус",
      education_hook:
        "объяснить продукт через сравнение: не палка колбасы, а коробка готовых вкусовых порций",
  ...
  if (/паштет/.test(lower)) {
    return {
      ...
      pain:
        input.pain ||
        "Паштет быстро теряет аккуратность, пачкает нож и превращается в безликую массу.",
      uniqueness:
        "Паштетки — мягкие паштетные подушечки для намазывания, где каждая порция уже собрана как отдельный кусок с ярким центром и не требует банки, ножа и повторного закрывания.",
      one_liner:
        "Паштет не в банке, а в мягких намазочных подушечках с готовой порцией на один раз.",
      physical_form:
        "мягкие прямоугольные паштетные подушечки в индивидуальных кармашках",
      appearance:
        "плоские гладкие подушечки с матовой поверхностью и контрастным центром на разломе",
      composition:
        "нежная печёночная или мясная база плюс сливочный, грибной или пряный центр",
      usage:
        "оторвать кармашек, раскрыть подушечку и намазать на хлеб без ножа и без банки",
      novelty_mechanism:
        "паштет превращён из общей массы в отдельные законченные намазочные единицы",
      why_people_will_try_it:
        "Это сразу считывается как новый формат паштета и снимает всю бытовую грязь категории.",
      new_market:
        "рынок аккуратных паштетных порций для дома, дороги и ланчбокса",
      replacement_logic:
        "заменяет баночный паштет, который пачкает приборы и быстро теряет аккуратность",
      ritual:
        "выбрать одну подушечку под завтрак, тост или перекус и не открывать весь объём сразу",
      ...
      launch_hook:
        "показать, как паштет впервые намазывается без банки и ножа",
      education_hook:
        "сравнить грязную банку и чистую мягкую порцию на один тост",
  ...
  return {
    name: normalizeName(
      input.name || fallbackNameFromCategory(input.category),
      input.category
    ),
    category: input.category,
    audience: input.audienceSummary,
    pain:
      input.pain ||
      "Потребителю нужен новый продукт без бытовой рутины и без ощущения, что он купил ещё одну одинаковую версию категории.",
    uniqueness:
      "Новый модульный продукт категории, где каждая единица уже собрана как отдельный предмет с собственной механикой использования и ярким образом.",
    one_liner:
      "Не обычный товар категории, а новый предмет с готовой порцией и своим ритуалом.",
    physical_form: "отдельные модульные порции фиксированной формы",
    appearance:
      "ровные законченные элементы с видимым отличительным признаком на поверхности или разрезе",
    composition:
      "базовая масса категории и контрастный вкусовой или функциональный элемент",
    usage:
      "взять одну готовую единицу, открыть, использовать без подготовки и оставить остальной объём закрытым",
    novelty_mechanism:
      "продукт перестаёт быть общей массой категории и становится набором готовых отдельных решений",
    why_people_will_try_it:
      "Форма считывается как новая вещь, а не как обычный товар в другой обёртке.",
    new_market:
      "подкатегория готовых модульных решений внутри основной категории",
    replacement_logic:
      "заменяет обычный продукт категории, который требует больше действий и хуже выглядит как самостоятельный предмет",
    ritual:
      "человек выбирает одну готовую единицу под конкретный момент использования",
    ...
    launch_hook:
      "сопоставить старый неудобный способ использования и новый предметный формат",
    education_hook:
      "объяснять через прямое сравнение старого и нового сценария",
...
function isWeakConcept(concept) {
  ...
  const required = [
    concept.name,
    concept.one_liner,
    concept.physical_form,
    concept.appearance,
    concept.composition,
    concept.usage,
    concept.novelty_mechanism,
    concept.why_people_will_try_it
  ];

  if (required.some((item) => sanitizeText(item).length < 10)) return true;

  const lowered = required
...
function mergeConcept(rawConcept, fallback, input) {
  ...
  const merged = {
    name: normalizeName(
      pickNonEmpty(source.name, input.name, fallback.name),
      input.category
    ),
    ...
    uniqueness: cleanupAnswer(
      pickNonEmpty(source.uniqueness, fallback.uniqueness)
    ),
    one_liner: cleanupAnswer(
      pickNonEmpty(source.one_liner, source.oneLiner, fallback.one_liner)
    ),
    physical_form: cleanupAnswer(
      pickNonEmpty(
        source.physical_form,
        source.physicalForm,
        fallback.physical_form
      )
    ),
    appearance: cleanupAnswer(
      pickNonEmpty(source.appearance, fallback.appearance)
    ),
    composition: cleanupAnswer(
      pickNonEmpty(source.composition, fallback.composition)
    ),
    usage: cleanupAnswer(pickNonEmpty(source.usage, fallback.usage)),
    novelty_mechanism: cleanupAnswer(
      pickNonEmpty(
        source.novelty_mechanism,
        source.noveltyMechanism,
        fallback.novelty_mechanism
      )
    ),
    why_people_will_try_it: cleanupAnswer(
      pickNonEmpty(
        source.why_people_will_try_it,
        source.whyPeopleWillTryIt,
        fallback.why_people_will_try_it
      )
    ),
    new_market: cleanupAnswer(
      pickNonEmpty(source.new_market, source.newMarket, fallback.new_market)
    ),
    replacement_logic: cleanupAnswer(
      pickNonEmpty(
        source.replacement_logic,
        source.replacementLogic,
        fallback.replacement_logic
      )
    ),
    ritual: cleanupAnswer(pickNonEmpty(source.ritual, fallback.ritual)),
    packaging: cleanupAnswer(
      pickNonEmpty(source.packaging, fallback.packaging)
    ),
    lineup: normalizeList(source.lineup, fallback.lineup),
    price_corridor: cleanupAnswer(
      pickNonEmpty(
        source.price_corridor,
        source.priceCorridor,
        fallback.price_corridor
      )
    ),
    visual_code: cleanupAnswer(
      pickNonEmpty(source.visual_code, source.visualCode, fallback.visual_code)
    ),
    sound_code: cleanupAnswer(
      pickNonEmpty(source.sound_code, source.soundCode, fallback.sound_code)
    ),
    aroma_code: cleanupAnswer(
      pickNonEmpty(source.aroma_code, source.aromaCode, fallback.aroma_code)
    ),
    tactile_code: cleanupAnswer(
      pickNonEmpty(
        source.tactile_code,
        source.tactileCode,
        fallback.tactile_code
      )
    ),
    taste_code: cleanupAnswer(
      pickNonEmpty(source.taste_code, source.tasteCode, fallback.taste_code)
    ),
    channels: normalizeList(source.channels, fallback.channels),
    launch_hook: cleanupAnswer(
      pickNonEmpty(source.launch_hook, source.launchHook, fallback.launch_hook)
    ),
    education_hook: cleanupAnswer(
      pickNonEmpty(
        source.education_hook,
        source.educationHook,
        fallback.education_hook
      )
    ),
    technology: cleanupAnswer(
      pickNonEmpty(source.technology, fallback.technology)
    )
  };
...
function extractAnswer(rawBlock, item, index) {
  ...
  if (Array.isArray(rawBlock)) {
    ...
    if (byNo) {
      return sanitizeText(byNo.answer ?? byNo.value ?? byNo.text ?? byNo);
    }
...
function generateBlockFallback(no, concept) {
  switch (no) {
    case "1.1":
      return `${concept.pain} ${concept.replacement_logic}. Продукт атакует боль не обещанием, а новой конструкцией: ${concept.one_liner}`;
    case "1.2":
      return `${concept.new_market}. Дополнительная монетизируемая ценность: ${concept.why_people_will_try_it}`;
    case "1.3":
      return `Новый ритуал: ${concept.ritual}. Механика использования: ${concept.usage}`;
    case "1.4":
      return `Нарратив строится так: ${concept.one_liner} Это выгодно, потому что ${concept.novelty_mechanism}`;
    case "1.5":
      return `${concept.education_hook}. Каналы обучения: дегустации, короткие видео, упаковка, социальное доказательство.`;
    case "2.1":
      return `${concept.appearance}. Визуальный код: ${concept.visual_code}`;
    case "2.2":
      return `${concept.sound_code}. Этот звук должен сопровождать первое использование.`;
    case "2.3":
      return `${concept.aroma_code}. Запах должен подтверждать чистоту и понятность продукта.`;
    case "2.4":
      return `${concept.tactile_code}. Тактильность должна поддерживать ощущение законченного предмета.`;
    case "2.5":
      return `${concept.taste_code}. Линейка вкусов: ${concept.lineup.join(", ")}.`;
    case "3.1":
      return `Продукт улучшает историю потребителя так: ${concept.why_people_will_try_it}`;
    case "3.2":
      return `Помогают контексты: ${concept.channels.join(", ")}. Мешают контексты, где решение принимают только по цене и не видят новизну формы.`;
    case "3.3":
      return `Название: ${concept.name}. Суть: ${concept.one_liner}. Слоган: ${concept.launch_hook}`;
    case "3.4":
      return `Путь клиента: увидеть образ, попробовать новый ритуал, закрепить повтор через удобство и вкус.`;
    case "3.5":
      return `3 года — закрепить формат; 5 лет — расширить линейку ${concept.lineup.join(", ")}; 10 лет — стать стандартом новой подкатегории.`;
    case "4.1":
      return `Ключевой сегмент: ${concept.audience}. Позиционирование: ${concept.one_liner}`;
    case "4.2":
      return `База: ${concept.one_liner}. Развитие: ${concept.lineup.join(", ")}.`;
    case "4.3":
      return `Ценовой коридор: ${concept.price_corridor}. Цена читается через новизну формы, вкуса и ритуала.`;
    case "4.4":
      return `Каналы: ${concept.channels.join(", ")}. Нужны места, где продукт можно быстро считать глазами.`;
    case "4.5":
      return `Запуск: ${concept.launch_hook}. Безбюджетно: демонстрации, короткие ролики, UGC, сравнение со старым способом.`;
    default:
      return concept.one_liner;
  }
}
...
function normalizeDraft(rawDraft, input, include, concept) {
  ...
  const draft = {
    header: {
      ...
      name: normalizeName(
        pickNonEmpty(rawDraft.header?.name, fallback.header.name),
        input.category
      ),
  ...
  Object.entries(getEnabledSchemas(include)).forEach(([key, items]) => {
    ...
    draft.blocks[key] = items.map((item, index) => ({
      ...
      answer: normalizeMeaningfulText(
        ...
        fallback.blocks[key][index].answer
      )
    }));
  });

  if (include.tech !== false) {
    draft.tech = normalizeList(rawDraft.tech, fallback.tech);
  }
  if (include.packaging !== false) {
    draft.packaging = normalizeList(rawDraft.packaging, fallback.packaging);
  }
  if (include.star !== false) {
    draft.star = normalizeList(rawDraft.star, fallback.star);
  }
  if (include.conclusion !== false) {
...
async function polishDraft(draft) {
  const polished = await callTextModel(
    [
      { role: "system", content: buildPolishSystemPrompt() },
      { role: "user", content: buildPolishUserMessage(draft) }
    ],
    POLISH_TEMPERATURE
  );
...
async function runGenerate(body = {}) {
  ...
  return normalizeDraft(polished, input, include, concept);
}
