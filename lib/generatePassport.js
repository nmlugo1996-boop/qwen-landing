...
const CONCEPT_TEMPERATURE = 0.55;
const PASSPORT_TEMPERATURE = 0.20;
const REQUEST_TIMEOUT_MS = 120000; // ms
...
// Новая функция: загрузка референсов style_examples
function loadStyleReferenceFiles(relDir = "style_examples", maxChars = MAX_REFERENCE_CHARS) {
  try {
    const baseDir = path.join(REF_DIRS[0], relDir);
    if (!fs.existsSync(baseDir) || !fs.statSync(baseDir).isDirectory()) return "";
    const names = fs.readdirSync(baseDir).filter(f => f.endsWith(".txt") || f.endsWith(".md")).sort();
    let acc = "";
    ...
    console.warn("[refs] failed to load style_examples", e && e.message);
    return "";
  }
}

// Основной эталон: prompt + schema + style_examples
...
const STYLE_EXAMPLES = truncateText(loadStyleReferenceFiles("style_examples", Math.max(0, MAX_REFERENCE_CHARS - 4000)), Math.max(0, MAX_REFERENCE_CHARS - 4000));
...
function buildPassportSystemPrompt(enabledSchemas, include) {
  ...
  return joinNonEmpty([
    ...
    "ОБЯЗАТЕЛЬНО (контроль качества вывода):",
    "- Для каждого пункта когнитивного/сенсорного/брендингового/маркетингового блока верни 3 абзаца, каждый абзац 2-4 предложения. В каждом ответе обязательно: 1) что это, 2) почему это важно (причина), 3) как это проявится в поведении потребителя (примеры сценариев).",
    "- Для каждой модальности сенсорики (визуал, звук, запах, осязание, вкус) дай 3-5 сенсорных якорей и одну мини-сценарную историю потребления (1-3 предложения).",
    "- Для бренд-блока дай: обещание бренда (1 фраза), 3 нарратива (каждый 1-2 предложения), путь клиента в 3 шага и план развития 3/5/10 лет (по 2 пункта на горизонт).",
    "- Указывай числа, метрики, и реальные предположения как [ПРЕДПОЛОЖЕНИЕ]. Если нет данных — указывай, какие данные нужны для проверки.",
    "- Запрещено копировать дословно из раздела 'Style examples'. Заимствуй глубину и структуру, но формулируй оригинально.",
    "- Если любой ответ короче 100 слов — пометь его как 'TOO_SHORT' (будет запрос на расширение).",
...
// ----------------- авто-расширение коротких ответов -----------------
async function expandShortAnswersIfAny(draftObj, include, minWords = 60) {
  if (!draftObj || typeof draftObj !== "object") return draftObj;
  const shortKeys = [];
  if (draftObj.blocks && typeof draftObj.blocks === "object") {
    for (const [blockKey, items] of Object.entries(draftObj.blocks)) {
      if (!Array.isArray(items)) continue;
      items.forEach((it, idx) => {
        const txt = String(it.answer || "").trim();
        const words = txt ? txt.split(/\s+/).length : 0;
        if (words < minWords) {
          shortKeys.push({ path: `${blockKey}[${idx}]`, block: blockKey, index: idx, no: it.no, question: it.question, current: txt });
        }
      });
    }
  }
  if (!shortKeys.length) return draftObj;

  const expandRequests = shortKeys.map(s => `- ${s.block}.${s.no} (${s.question}): current answer is too short. Please expand into 3 paragraphs (2-4 sentences each), include cause→effect and one consumption scenario. Mark assumptions as [ПРЕДПОЛОЖЕНИЕ].`).join("\n");

  const roleSystemFollow = buildPassportSystemPrompt(getEnabledSchemas(include), include);
  const roleUserFollow = `${JSON.stringify({ header: draftObj.header, product_core: draftObj.product_core })}\n\nДополняющие инструкции:\n${expandRequests}\n\nВерни ТОЛЬКО JSON с полем blocks (только те блоки/ответы, которые ты расширил можно вернуть).`;

  const follow = await callTextModel([{ role: "system", content: roleSystemFollow }, { role: "user", content: roleUserFollow }], 0.18);
  if (!follow || typeof follow !== "object") return draftObj;

  try {
    const updated = JSON.parse(JSON.stringify(draftObj)); // клонируем
    const followBlocks = follow.blocks || follow;
    for (const [bkey, items] of Object.entries(followBlocks || {})) {
      if (!Array.isArray(items)) continue;
      items.forEach(fItem => {
        const target = (updated.blocks && updated.blocks[bkey]) ? updated.blocks[bkey].find(i => String(i.no) === String(fItem.no)) : null;
        if (target && fItem.answer && String(fItem.answer).trim().length > String(target.answer || "").trim().length) {
          target.answer = cleanupAnswer(fItem.answer);
        }
      });
    }
    return updated;
  } catch (e) {
    console.warn("[generate] expandShortAnswersIfAny merge failed", e && e.message);
    return draftObj;
  }
}
...
async function createPassport(input, include, concept) {
  ...
  // попробуем расширить короткие ответы сразу после получения raw
  const rawExpanded = await expandShortAnswersIfAny(raw, include, 60); // порог можно поднять
  const v = validatePassportObject(rawExpanded);
  if (!v.ok) {
    ...
    const roleUserFollow = `Текущий черновик паспорта: ${JSON.stringify(rawExpanded)}. Недостающие поля: ${JSON.stringify(v.missing)}.`;
    const filled = await fillMissingFields(rawExpanded, v.missing, roleSystemFollow, roleUserFollow, 0.18);
  ...
  return normalizeDraft(rawExpanded, input, include, concept);
}
